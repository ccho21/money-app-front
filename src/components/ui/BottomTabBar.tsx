...
const tabs = [
  { label: '03-24', icon: <BookText className="w-5 h-5" />, path: '/dashboard/daily' },
...
export default function BottomTabBar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-zinc-900 border-t flex justify-around py-2">
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            <span className={isActive ? 'text-red-500 font-semibold' : ''}>{tab.label}</span>
          </button>
        );
      })}
    </nav>
